...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        # Verificar valores usando numpy.testing.assert_allclose
        npt.assert_allclose(result, expected, rtol=1e-7, atol=1e-7)
        
        # Verificar forma del resultado (len(arr) - window + 1 = 4 - 2 + 1 = 3)
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        # Test con window=0 (valor no positivo)
        with self.assertRaises(ValueError):
            utils.moving_average(arr, window=0)
        
        # Test con window negativo
        with self.assertRaises(ValueError):
            utils.moving_average(arr, window=-1)
        
        # Test con window mayor que la longitud del array
        with self.assertRaises(ValueError):
    ...
    def test_moving_average_only_accepts_1d_sequences(self):
        ...
        # Verificar que se lanza ValueError para secuencia 2D
        with self.assertRaises(ValueError):
    ...
    def test_zscore_raises_for_zero_std(self):
        ...
        # Verificar que se lanza ValueError cuando todos los valores son iguales
        with self.assertRaises(ValueError):
            utils.zscore(arr)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        # Verificar valores transformados: (x - min) / (max - min)
        # min=2, max=6, range=4
        # [2->0.0, 4->0.5, 6->1.0]
